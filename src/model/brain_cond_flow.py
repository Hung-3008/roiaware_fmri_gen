"""
BrainCondFlow — Direct Conditional OT Flow Matching Architecture.

Single-branch model: DINOv2 features condition a flow model that
matches directly from N(0,I) to z_true (the full fMRI latent).

Supports two backbones:
  1. DiT (default) — AdaLN-Zero blocks, inspired by MOTFM (MICCAI 2025)
  2. Transformer   — Standard TransformerEncoder with time-concatenation,
                     inspired by FlowNP (NeurIPS 2025)

Architecture:
  ContextEncoder         — Learnable multi-layer DINOv2 mixing
  FlowPrefixDiT          — DiT blocks with [context || latent] prefix attention + AdaLN-Zero
  FlowTransformerEncoder — Standard TransformerEncoder with time concat (FlowNP-style)
"""
...
class CondFlowDiTBlock(nn.Module):
...
class BrainCondFlowConfig:
    ...
    context_dim: int = 768
    n_dino_layers: int = 4
    n_context_queries: int = 257

    # Flow backbone: "dit" (AdaLN-Zero) or "transformer" (FlowNP-style)
    backbone: str = "dit"

    # Flow (DiT / Transformer)
...
class ContextEncoder(nn.Module):
    """Mixes multi-layer DINOv2 via learnable weights."""
    def __init__(self, config: BrainCondFlowConfig):
        super().__init__()
        self.config = config
...
class FlowPrefixDiT(nn.Module):
    """Prefix DiT matching noise → z_true directly."""
    def __init__(self, config: BrainCondFlowConfig):
        super().__init__()
        self.config = config
        ...
        self.blocks = nn.ModuleList([
            CondFlowDiTBlock(
                ...
                mlp_ratio=config.mlp_ratio, dropout=config.dropout, drop_path_rate=dpr[i]
...
class FlowTransformerEncoder(nn.Module):
    """FlowNP-style: standard TransformerEncoder with time concatenation.
    Time is injected by concatenating sinusoidal embedding to latent tokens,
    rather than using AdaLN modulation."""
    def __init__(self, config: BrainCondFlowConfig):
        super().__init__()
        self.config = config
        D = config.flow_hidden_dim
        ...
        # Latent tokens: token_dim + time_emb_dim -> D
        assert config.latent_dim % config.n_latent_tokens == 0
        self.token_dim = config.latent_dim // config.n_latent_tokens
        self.time_emb_dim = D  # sinusoidal time embedding dimension
        self.latent_embedder = nn.Sequential(
            nn.Linear(self.token_dim + self.time_emb_dim, D),
            nn.ReLU(True),
            nn.Linear(D, D),
        )
        self.latent_pos_embed = nn.Parameter(torch.randn(1, config.n_latent_tokens, D) * 0.02)
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=D, nhead=config.flow_num_heads,
            dim_feedforward=int(D * config.mlp_ratio),
            dropout=config.dropout, activation="gelu",
            batch_first=True, norm_first=True,
        )
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=config.flow_depth)

        # Output predictor (MLP)
        self.predictor = nn.Sequential(
            nn.Linear(D, int(D * config.mlp_ratio)),
            nn.ReLU(),
            nn.Linear(int(D * config.mlp_ratio), self.token_dim),
        )
    ...
    def forward(self, t, z_t, context, mask_ratio=0.0):
        ...
        # Latent tokens with time concatenation (FlowNP-style)
        z_seq = z_t.view(B, N, self.token_dim)
        t_emb = timestep_embedding(t, self.time_emb_dim)
        t_emb_expanded = t_emb.unsqueeze(1).expand(B, N, self.time_emb_dim)
        z_with_t = torch.cat([z_seq, t_emb_expanded], dim=-1)
        x = self.latent_embedder(z_with_t) + self.latent_pos_embed

        # Concat context + latent, process with TransformerEncoder
        seq = torch.cat([c, x], dim=1)
        seq = self.encoder(seq)

        # Extract latent portion and predict velocity
        x_out = seq[:, -N:, :]
        v_pred = self.predictor(x_out)
        return v_pred.reshape(B, -1)
    ...
    def forward_with_cfg(self, t, z_t, context, cfg_scale=1.0):
        ...
        # Latent tokens with time (doubled)
        z_seq = z_t.view(B, N, self.token_dim)
        t_emb = timestep_embedding(t, self.time_emb_dim)
        t_emb_expanded = t_emb.unsqueeze(1).expand(B, N, self.time_emb_dim)
        z_with_t = torch.cat([z_seq, t_emb_expanded], dim=-1)
        ...
        x_out = seq[:, -N:, :]
        v_pred = self.predictor(x_out).reshape(2 * B, -1)

        v_cond, v_uncond = v_pred.chunk(2, dim=0)
        return v_uncond + cfg_scale * (v_cond - v_uncond)


class BrainCondFlow(nn.Module):
    """Direct conditional OT flow matching: DINOv2 -> z_true.
    No regression branch, no residual decomposition.
    Supports backbone='dit' (AdaLN-Zero) or 'transformer' (FlowNP-style)."""
    def __init__(self, config: BrainCondFlowConfig):
        super().__init__()
        self.config = config
        self.context_encoder = ContextEncoder(config)
        if config.backbone == "transformer":
            self.flow_branch = FlowTransformerEncoder(config)
        else:
